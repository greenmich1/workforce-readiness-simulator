...
def optimize(snapshot, time_limit=TIME_LIMIT_SEC):
...
def optimize_stream(snapshot, callback, time_limit=TIME_LIMIT_SEC):
    """
    Streaming solve.
    callback(snap_copy, score, elapsed_s, done=False) is called for each
    intermediate solution found, then once more with done=True for the final.
    """
    if _ORTOOLS:
        return _cpsat_solve(snapshot, callback=callback, time_limit=time_limit)
    result = _greedy_solve(snapshot)
    score  = result["metrics"].get("score", 40)
    ...
    if not feasible:
        ...
        if callback is not None:
            callback(copy.deepcopy(result),
                     result["metrics"].get("score", 40),
                     time.monotonic() - t0, done=True)
        return result

    label = "cpsat_optimal" if status == cp_model.OPTIMAL else "cpsat_feasible"
    _apply_solution(
        placements, session_vars, session_plan, course_data,
        S, solver, orig_emp_days, snapshot,
        time.monotonic() - t0, label, max_cls, num_rooms
    )

    if callback is not None:
        callback(copy.deepcopy(snapshot),
                 snapshot["metrics"].get("score", 40),
